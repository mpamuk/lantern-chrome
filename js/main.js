...
var User = Backbone.Model.extend({
    ...
    attemptLogin: function(callback) {
        ...
        if (callback !== undefined) {
            ...
            console.log("ATTEMPT LOGIN ELSE");
            console.log("Access token " + localStorage.accessToken);

            var data = $.ajax({
                type: "GET",
                url: getLoginUrl() + "?accessToken=" + localStorage.accessToken.substring(13),
                async: false
            }).responseText;
            var responseObj = JSON.parse(data);
            localStorage.userId = responseObj.payload.user.user_id;
            console.log(responseObj.payload.user.fb_name);
            console.log("USER ID REGISTERED " + localStorage.userId);
            var isLoggedIn = true;
            //this.attemptSetCSRF(data);
            return isLoggedIn;
        }
...
function openItem(tabId, url, favIconUrl, title, event_type) {
    ...
    console.log("USER INCOGNITO " + user.getIncognito());
    if (user.getIncognito() === false) {
        ...
        if (user.inWhitelist(url)) {
            console.log("CAME TO WHITELIST URL");
            if (timeCheck.allow) {
...
function closeItem(tabId, url, event_type, time) {
    ...
    //!user.inBlackList(url) &&
    if (activeItem.tabId === tabId && total_time > 5000) {
...
function getAPIUrl(resource, id, params) {
    var apiBase = "http://whitelist-api.herokuapp.com/history"
    /*params = params || {};
